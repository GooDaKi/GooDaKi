...
class Subject:
    def __init__(self, db_row, chunks):
        if db_row is not None:
            self.subjectID = db_row[0]
            self.subjectName = db_row[1]
    ...
    def get_by_author_id(authorid,limit_no=2, scroll_no=0, all=True):
        ...
        for subject in temp:
            if not bool(subject[6]):
                continue
            cursor = g.db.cursor()
            cursor.execute('SELECT chunkID FROM "ChunkInSubject" WHERE subjectID = (%s) ORDER BY ordering', (subject[0],))
            if cursor.rowcount < 0:
                cursor.close()
                return Subject(ret, None)
            chunks = list(cursor.fetchall())
